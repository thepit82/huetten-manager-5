...
// Default age categories as defined in the spec
...
export function AgeCategoryEditor({ tripId, categories, onChange }: AgeCategoryEditorProps) {
  ...
  async function handleResetToDefault() {
    ...
    try {
      ...
      // Insert defaults
      const inserts = DEFAULT_CATEGORIES.map((cat, i) => ({
        ...cat,
        trip_id: tripId,
        sort_order: i,
      }))
      ...
      if (error) throw error
      onChange(data ?? [])
      success('Altersklassen zurückgesetzt', 'Standard-Kategorien wurden wiederhergestellt')
    } catch (err) {
  ...
  return (
    <div>
              ...
              .map((ac, idx) => (
                <tr key={ac.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <span className="font-mono">{Number(ac.overnight_factor).toFixed(2)}</span>
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <span className="font-mono">{Number(ac.meal_factor).toFixed(2)}</span>
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <span className="font-mono">{Number(ac.food_factor).toFixed(2)}</span>
                  </td>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Input
              ...
              max="2"
              step="0.01"
              value={form.overnight_factor}
              onChange={setField('overnight_factor')}
              hint="0 = kostenlos"
            />
            <Input
              label="VP-Faktor"
              ...
              max="2"
              step="0.01"
              value={form.meal_factor}
              onChange={setField('meal_factor')}
            />
            <Input
              label="Essen-Faktor"
              type="number"
              ...
              max="2"
              step="0.01"
              value={form.food_factor}
              onChange={setField('food_factor')}
              hint="Mengenkalkulation"
            />
          </div>
      ...
      <ConfirmModal
        ...
        message={`Soll die Altersklasse „${deleteTarget?.name}" wirklich gelöscht werden? Personen, die dieser Klasse zugeordnet sind, verlieren die Zuordnung.`}
